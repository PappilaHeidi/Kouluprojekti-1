...
# Sovelluksen pääosa
def main():
    tab1, tab2 = st.tabs(["Sääsivu", "Tilastosivu"])
    with tab1:
    ...
    with tab2:
        ...
        df = df.sort_values(by="Asiakasmäärä", ascending=False)

        # Suurimman asiakasmäärän kuukaudet
        max_customers = df[df["Asiakasmäärä"] == df.iloc[0]["Asiakasmäärä"]]["Kuukausi"].tolist()

        st.write("*Suosituimmat kuukaudet pinkillä värillä*")
        bars = alt.Chart(df).mark_bar().encode(
            x='Kuukausi',
            y='Asiakasmäärä',
            color=alt.condition(
                alt.FieldOneOfPredicate(field='Kuukausi', oneOf=max_customers),
                alt.value('#FF91A4'),
                alt.value('#4169E1')
            )
        )
